...
export class FullVaultMetricsCollector {
	...
	// Batch size is adaptive: large when draining a fresh backlog (initial
	// mass-load), small once we are following live edits. A larger batch is
	// safe during mass-load because we also skip the per-tick graph recompute
	// until the backlog is nearly drained — see processBacklog().
	private batchSize: number = 16;
	private intervalMs = 100;
	// Threshold below which we resume per-tick orphan/trace recompute. While
	// the backlog is above this, those derivatives are stale anyway (most
	// notes still pending), so paying O(L) per tick is wasted work that
	// jitters the UI. metadataCache "resolved" still triggers a generation
	// bump, so we don't miss an update.
	private readonly graphRecomputeThreshold = 100;
	...
	private rescan() {
		...
		// Inline the backlog fill to avoid 1 clearInterval+setInterval pair per
		// file via push() — on a 6k-file vault that synchronous churn cost
		// ~hundreds of ms inside onload. The single setAdaptiveInterval call
		// below schedules processing once the backlog is fully assembled.
		// getMarkdownFiles() instead of getFiles() skips images/PDFs/etc — the
		// collector returns null for non-md anyway, so we save 1 backlog entry
		// per non-note attachment.
		for (const file of this.vault.getMarkdownFiles()) {
			this.backlog.add(file.path);
		}
		this.refreshTagCount();
		this.setAdaptiveInterval();
	}

	// Public because the contract is: update() no longer auto-refreshes the
	// tag count to keep per-file work cheap during mass-load. Callers that
	// need a fresh count (processBacklog batch tail, integration tests)
	// invoke this explicitly at their natural granularity.
	public refreshTagCount() {
	...
	// Handle for the next scheduled scan slot. Either an idle-callback
	// handle (mass-load path) or a setTimeout handle (live-edit path).
	private scanScheduled: { kind: 'idle'; id: number } | { kind: 'timeout'; id: number } | null = null;
	private setAdaptiveInterval() {
		// Cancel any pending schedule — we re-arm at the end of every batch.
		if (this.scanScheduled) {
			if (this.scanScheduled.kind === 'idle') {
				(window as Window & { cancelIdleCallback?: (h: number) => void }).cancelIdleCallback?.(this.scanScheduled.id);
			} else {
				window.clearTimeout(this.scanScheduled.id);
			}
			this.scanScheduled = null;
		}
		// Clean up the legacy interval if anything else set it.
		...
		// Strategy:
		//   * Large backlog (initial mass-load) → requestIdleCallback. The
		//     browser fires us only when the main thread has spare cycles,
		//     so we stop competing with Obsidian's own workspace restore
		//     (which was the cause of the 50–100 ms tick spikes in the
		//     first 1–2 seconds of scanning). Per-tick budget is bounded
		//     by the idle deadline; per-batch sync work stays small.
		//   * Small backlog (live edits) → setTimeout at a relaxed cadence.
		//     We don't need rIC overhead for the common case of "user just
		//     edited one note" — one short timeout is enough.
		//
		// requestIdleCallback is part of the WindowOrWorkerGlobalScope spec
		// and present in Electron (Obsidian's runtime). The optional chain
		// below preserves correctness if a future runtime omits it.
		const ric = (window as Window & { requestIdleCallback?: (cb: (d: IdleDeadline) => void, opts?: { timeout: number }) => number }).requestIdleCallback;
		if (backlogSize > 50 && typeof ric === 'function') {
			this.batchSize = 16;
			// `timeout: 1000` is the safety valve — if the user is doing
			// heavy continuous work (typing, scrolling endlessly), the
			// browser would otherwise never fire idle callbacks. After
			// 1 s we force-fire to make sure the scan still progresses.
			this.scanScheduled = {
				kind: 'idle',
				id: ric.call(window, () => {
					this.scanScheduled = null;
			...
			this.batchSize = backlogSize < 10 ? 16 : 16;
			const delay = backlogSize < 10 ? 1000 : 250;
			this.scanScheduled = {
				kind: 'timeout',
	...
	// Re-entrancy guard: when setInterval or requestIdleCallback fires we
	// schedule processBacklog asynchronously; another tick could fire while
	// the previous batch is still awaiting Promise.allSettled, and the two
	// invocations would race on the backlog Set. Skip if one is in flight.
	...
	private async processBacklog() {
		...
		try {
			...
			try {
				// Fan out all files in parallel: cachedRead() is IO-bound for
				// files Obsidian has not paged in yet, and chunking down the
				// parallelism here serialises that IO — measured as a 3-4x
				// regression on cold batches in a 6k-note vault.
				...
				// Compute graph-derived metrics in the batch tail so the
				// coalesced 'updated' event carries fresh orphan and trace
				// counts. update() already bumped the memoization generation
				// per file, so these computes run on a fresh gen and their
				// results are cached for the debounced view render that
				// follows.
				//
				// During an initial mass-load (backlog still huge), skip
				// these O(L) graph walks: most notes are still pending so
				// the numbers would be misleading anyway, and recomputing
				// 100-400× in a row freezes the UI. Resume once the backlog
				// is small (~final batches) so the user still sees the
				// converged value without waiting for the next live edit.
				if (this.metadataCache && this.backlog.size < this.graphRecomputeThreshold) {
				...
				// Refresh tag count once per batch (instead of per file inside
				// update()) so a 16-file batch makes 1 metadataCache.getTags()
				// walk instead of 16. On a 6k-file vault this per-update
				// refresh was the main cause of mid-scan UI jank: each
				// update() bumps the generation, invalidating the cache, so
				// every refresh forced a fresh O(tags) walk.
				this.refreshTagCount();
			} finally {
	...
	public async collect(file: TFile): Promise<CollectResult | null | undefined> {
		...
		//
		// Note: an earlier iteration inserted a setTimeout(0) yield here between
		// IO and CPU-bound sync work. Logging revealed that during user actions
		// (tab switches, big-note renders) those 24 macrotask yields per batch
		// would all serialise behind the user's render work, ballooning a
		// single tick to 6+ seconds and stalling the entire scanner via the
		// inFlight guard. Result: brief 50-70 ms sync bursts (acceptable) were
		// traded for multi-second user-perceived freezes (unacceptable).
		// Smaller batches now keep each burst short instead.
	...
	public update(fileOrPath: TFile | string, resultOrMetrics: CollectResult | FullVaultMetrics | null, _tags?: Set<string>) {
		...
		// NOTE: tag count refresh is intentionally NOT called per file. During
		// a mass-load every update() bumps the generation, which invalidates
		// tagOccurrencesCache; calling refreshTagCount() here would force a
		// fresh metadataCache.getTags() walk per file — O(tags×files) per
		// batch. Callers (processBacklog, single-file event handlers) refresh
		// the tag count once at the right granularity instead.
	}
