 ...
 *
 * Entries still carrying English text are placeholders: the catalogue is being
 * filled in call-site by call-site, and the translation pass lands as its own
 * change. A test asserts that no English is left once that pass is done.
 */
export const ru: Translations = {
	...
	settings: {
		...
		on: "On",
		off: "Off",
		ofTotal: (enabled, total) => `${enabled} of ${total}`,
		vaultRoot: "(vault root)",
		listEmpty: (addLabel) => `Nothing yet — use “${addLabel}”.`,
		pickNote: (name) => `Pick a note — ${name}`,
		pickFolder: (name) => `Pick a folder — ${name}`,
		...
		counting: {
			...
			classificationValue: (own, source) => `${own} own / ${source} source`,
			ownTags: "Own tags",
	...
	view: {
		...
		hero: {
			...
			words: "words",
			QoV: "QoV",
			wordsTooltip: (total) => `Total words across the vault: ${total}`,
		...
		metrics: {
			...
			avgWords: "avg words",
			avgWordsTooltip: (avg) => `Average words per note: ${avg} (words ÷ notes)`,
		},
		...
		inbox: {
			...
			folders: (n) => `${n} inbox folders`,
			tag: (tag) => `#${tag} (outside inbox)`,
			tags: (n) => `${n} review tags (outside inbox)`,
		...
		folders: {
			...
			ownSourceTooltip: (own, source) => `${own} own · ${source} source`,
		},
		...
		history: {
			...
			orphans: "orphans",
			traced: "traced",
			delta: (delta) => `${delta > 0 ? "+" : ""}${delta} notes`,
	...
	tangles: {
		...
		count: (n) => `${n} ${n === 1 ? "tangle" : "tangles"}`,
		empty: "No notes match the current tangle thresholds. Lower min in / min out in settings, or remove an exclude entry.",
		legendIn: "incoming",
		legendOut: "outgoing",
		badgeTitle: (inCount, outCount) => `${inCount} incoming · ${outCount} outgoing`,
		exclude: "Exclude from tangles",
		modeSum: (total) => `sum · in+out ≥ ${total}`,
		modeAndOr: (mode, minIn, op, minOut) => `${mode} · in ≥ ${minIn} ${op} out ≥ ${minOut}`,

		reportFileName: (date) => `Vault Tangles — ${date}.md`,
		reportTitle: (date) => `# Vault Tangles — ${date}`,
		reportMode: (mode) => `- Mode: ${mode}`,
		reportTotal: (n) => `- Total tangles: ${n}`,
		reportEmpty: "_No notes match the current tangle thresholds._",
		reportTableHead: "| In | Out | Note |",
		reportModeSum: (total) => `\`sum\` · in+out ≥ ${total}`,
		reportModeAndOr: (mode, minIn, op, minOut) => `\`${mode}\` · in ≥ ${minIn} ${op} out ≥ ${minOut}`,
	},

	inbox: {
		reportTitle: (date) => `## Inbox health — ${date}`,
		ageFresh: "fresh (<1d)",
	...
	notices: {
		...
		exported: (n, target) => `Exported ${n} snapshot${n === 1 ? "" : "s"} to ${target}`,
		exportFailed: (reason) => `Export failed: ${reason}`,
		excludedFromTangles: (path) => `Excluded "${path}" from tangles`,
		folderCreateFailed: (folder, reason) => `Could not create folder "${folder}": ${reason}`,
		tanglesSaved: (n, path) => `Saved ${n} tangle${n === 1 ? "" : "s"} to ${path}`,
		tanglesReportFailed: (reason) => `Tangles report failed: ${reason}`,
	},
};
